...
public class Leveyshaku {
     ...
     * @param maali taulukko nodeja siten, että indeksissä i on maalin sijainti
     * i askeleen jälkeen
     ...
     * @return Noden kustannus, mikäli se on pienempi kuin noden indeksi
     * maalilistassa. Muussa tapauksessa Integer.MAX_VALUE
     */
    private static int moneskoReitilla(Node node, Node[] maalit) {
        for (int i = node.getKustannusNodeenAsti() + 1; i < maalit.length; i++) {
